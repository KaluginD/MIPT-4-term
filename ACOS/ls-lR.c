...
const int FLAGS[9] = { S_IRUSR, S_IWUSR, S_IXUSR, S_IRGRP, S_IWGRP, S_IXGRP, S_IROTH, S_IWOTH, S_IXOTH };
...
void make_rights_formatted (int st_mode, char result[11]) {
  ...
  for (int i = 1; i < 10; i++)
    if ((st_mode & FLAGS[i - 1]) == FLAGS[i - 1])
      result[i] = SYMBOLS[i - 1];
    else
      result[i] = WRONG_SYMBOL;
}
...
void ls (char *dirname) {
  ...
  qsort(entries, entries_count, sizeof(char *), str_compare);
...
int main (int argc, char **argv) {
  char **max_dir = argv + argc;
  for (char **dir = argv + 1; dir < max_dir; dir++) {
    if (dir > argv + 1) {
      printf("\n");
    }
    ls(*dir);
